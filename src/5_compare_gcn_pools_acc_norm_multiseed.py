...
EPOCHS = 400
PATIENCE = 50
VAL_METRIC = "accuracy"  # "macro_f1" or "accuracy"
...
def main():
    ...
    # Row-normalize features (like T.NormalizeFeatures())
...
# --------- Multi-seed runner with normalized features + accuracy early-stop ---------
def run_one_seed(seed):
    set_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Load Cora and normalize features
    data = load_cora_from_content_and_cites(CORA_CONTENT, CORA_CITES)
    data.x = data.x.to(torch.float)
    row_sum = data.x.sum(dim=1, keepdim=True).clamp(min=1e-12)
    data.x = data.x / row_sum

    # Planetoid-style split (seeded)
    train_mask, val_mask, test_mask = make_planetoid_style_split(
        data.y, data.num_classes, train_per_class=20, val_size=500, test_size=1000
    )
    ...
    # 2) DiffPool (K matched to L-RMC for fairness)
